    ...
    <div class="footer-bar">
      <ul class="posters-list">
        ...
        >
          ...
          <div
            class="fz-12 text-center"
            :class="active == index ? 'text-primary':'c9'"
          >{{item.title}}</div>
        </li>
      </ul>
    </div>
...
export default {
  ...
  methods: {
    async getData() {
      ...
      let { code, data, message } = await axios.get("/course/detail", {
    ...
    async getPosters() {
      let { code, data, message } = await axios.get("/course/share-temp", {
        params: { id: this.$route.params.id }
      });
      if (code == 0) {
        if (data.length) {
