<template>
  <div class="plr-15 pt-10">
    ...
    <van-grid :column-num="5" :border="false">
      <van-grid-item v-for="(item,index) in tool" :key="index" :icon="item.image" :text="item.title" :to="{ name:'tool_detail', params: {id: item.id}}" />
    </van-grid>
    <!-- 菜单 -->
    ...
    methods: {
      ...
      async login() {
        let {
          code,
          data
        } = await window.axios.get("/user/login?id=356");
        if (code == 0) {
          this.getData();
          this.getCourseHot();
      ...
      async checkLogin() {
        ...
        if (code == 0) {
          ...
          if (isDev) {
            ...
            // 如是子公司进入授权带上c_code
            if (this.c_code) {
              window.location.href = `http://youya.chuncom.com/user/authorization?url=${href}&c_code=${this.c_code}`;
            } else {
      ...
      async getData() {
        ...
        let params = {};
        if (this.c_code) {
          params.company_code = this.c_code;
        }
        let {
          code,
          data,
        ...
        if (code == 0) {
          ...
          if (data.company) {
            ...
            document.querySelector('link[rel="icon"]').href = data.company.image;
          }
        } else {
