<template>
  <div>
    ...
    <div v-if="list.length" class="goods-item" v-for="(item,index) in list" :key="index">
      <div class="checkbox">
        <van-checkbox v-if="!changeEdit" v-model="item.checked" checked-color="#ee7063" @click="checkedChange" :data-index="index"></van-checkbox>
        <van-checkbox v-if="changeEdit" v-model="item.editCkecked" checked-color="#ee7063" @click="checkedChange2" :data-index="index"></van-checkbox>
      </div>
      ...
      <div class="ml-10 flex flex-column flex-jus" style="flex:1;">
        ...
        <div class="flex flex-jus flex-align-end">
          <div class="fz-15 text-price">¥{{item.with_format_price ? item.with_format_price.price : item.with_goods.price}}</div>
    ...
    methods: {
      ...
      async getCartList() {
        ...
        let {
          ...
          data,
          message
        } = await axios.get("/user/mall-cart")
        if (code == 0) {
          this.$toast.clear();
          this.list = data
      ...
      async deleteItem() {
        ...
        let {
          code,
          message
        } = await axios.post("/user/mall-cart/delete", {
          ids: goodsList.join(",")
        })
        if (code == 0) {
      ...
      clearing() {
        ...
        this.$store.commit("setCreateOrderGoodsList", goodsList);
        this.$router.push({
