      ...
      <ul class="reservation-list">
        <li class="reservation-item" v-for="(item,index) in list" :key="item.id">
          <div class="flex flex-jus mb-10">
            ...
            <div v-if="item.status == 0" class="fz-13 text-price">待消课</div>
            <div v-if="item.status == 1" class="fz-13 text-price">已完成</div>
            <div v-if="item.status == 2" class="fz-13 text-price">已过期</div>
            <div v-if="item.status == 3" class="fz-13 text-price">已取消</div>
          </div>
          ...
          <div v-if="item.status != 3" class="flex flex-end">
            <div @click="popupPic" :data-qr="item.qr" class="btn-youya">消课码</div>
...
<script>
  export default {
    ...
    methods: {
      popupPic(e){
        this.qrPic = e.currentTarget.dataset.qr;
        this.show = true;
      },
      async getList() {
        this.$toast.loading({
        ...
        if(this.active - 1 >=0){
          var {
            ...
            data,
            message
          } = await axios.get(`/reservation/list?page=${this.page++}&status=${this.active - 1}`);
        }else{
          var {
            code,
            data,
            message
          } = await axios.get(`/reservation/list?page=${this.page++}`);
        }
        if (code == 0) {
          this.$toast.clear()
          this.list = [
